...
pub(crate) fn get_trampoline_path() -> Result<vite_path::AbsolutePathBuf, Error> {
    ...
    if let Ok(override_path) = std::env::var("VITE_PLUS_TRAMPOLINE_PATH") {
...
pub(crate) async fn cleanup_legacy_windows_shim(bin_dir: &vite_path::AbsolutePath, tool: &str) {
    ...
    if tokio::fs::try_exists(&cmd_path).await.unwrap_or(false) {
        ...
        tracing::debug!("Removed legacy .cmd wrapper {:?}", cmd_path);
    }
    ...
    if tokio::fs::try_exists(&sh_path).await.unwrap_or(false) {
        // Read first bytes to check if it's a shell script
        if let Ok(content) = tokio::fs::read(&sh_path).await {
            if content.starts_with(b"#!/bin/sh") || content.starts_with(b"@echo off") {
                ...
                tracing::debug!("Removed legacy shell wrapper {:?}", sh_path);
            }
        }
    }
}
