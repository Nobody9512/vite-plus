...
fn is_potential_package_binary(tool: &str) -> bool {
    ...
    // On Unix: shims are symlinks (e.g., bin/tsc)
    // On Windows: shims are trampoline .exe files (e.g., bin/tsc.exe)
    let shim_path = configured_bin.join(tool);
    if std::fs::symlink_metadata(&shim_path).is_ok() {
        return true;
    }

    // On Windows, also check for .exe extension (trampoline shims)
    #[cfg(windows)]
    {
